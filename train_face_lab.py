...
def main_face_label(dataset, algo):
    ...
    for net in ["TFCNN"]:  # Different types of CNNs. We will train all the differenet clients in in each one of them one-by-one
        for local_epochs in [2, 5]: # iterate over 2 then 5 only
            for j in range(hypers["cnt"]): # iterate over the no. of possible values for the hyperparameters 
                ...
                para_dict["n_clients"] = 20  # Specify the number of total clients.
                para_dict["c_ratio"] = 0.2 # c_ratio * n_clients = no. of clients that will participate in a single global training round
                para_dict["local_epochs"] = local_epochs
                para_dict["max_round"] = 400
