...
from datasets.face_data import load_face_data, FaceDataset
...
class FedData():
    ...
    def __init__(
        ...
        self.label_dsets = [
            "famnist",
            "cifar10", "cifar100",
            "tumor4", "tumor2", "DAC", "Swipe","Voice", "face"
        ]
    ...
    def construct(self, n_clients=None, nc_per_client=None):
        """ load raw data
        """
        ...
        # Load data and split the data for all the clients
            ...
            # The label data (train_ys and test_ys) are tensors of dimension (N, ). Where, N =  no. of images in that batch(or the no. of labels), and the label themselves (what
        ...
        elif self.dataset in ["face"]: 
            # Say you want to get the first image's label value. Then, train_ys[0] will return a 1D tensor containing the integer value representing the actual label
            train_xs, train_ys, test_xs, test_ys = load_face_data( 
                self.dataset, combine=False
            )
            clients_data = self.split_by_label_noniid(train_xs, train_ys)
            csets, gset = self.construct_datasets(
                clients_data, FaceDataset, test_xs, test_ys
            )
        elif self.dataset in ["DAC","Swipe","Voice"]: 
            # Say you want to get the first image's label value. Then, train_ys[0] will return a 1D tensor containing the integer value representing the actual label
            if n_clients is not None:
                train_xs, train_ys, test_xs, test_ys = load_behavioral_data(
                    self.dataset, combine=False, n_clients=n_clients, nc_per_client=nc_per_client
                )
            else:
                train_xs, train_ys, test_xs, test_ys = load_behavioral_data(
                    self.dataset, combine=False
