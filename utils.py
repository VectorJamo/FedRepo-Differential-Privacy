...
#load the png files with directory as lable
def load_pngs_with_labels(directory):
    images_with_labels = []
    if os.path.isdir(directory):  # Check if the path is a valid directory
        folder_name = os.path.basename(directory)  # Get the folder name
        for filename in os.listdir(directory):
            if filename.endswith(".png"):  # Check for .png files
                img_path = os.path.join(directory, filename)
                img = Image.open(img_path)
                images_with_labels.append((img, folder_name))  # Store image and label
